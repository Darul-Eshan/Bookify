@php
    $headerCartItems = Auth::check() ? \App\Models\Cart::where('user_id', Auth::id())->latest()->get() : collect();
    $headerCartCount = $headerCartItems->sum('quantity');
    $headerCartSubtotal = $headerCartItems->sum(fn($item) => $item->price * $item->quantity);
@endphp
    ...
    <div class="max-w-7xl mx-auto flex items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-4">
            ...
            <a href="#" class="relative text-gray-300 hover:text-white p-2.5 rounded-xl bg-[#161626] border border-gray-800 transition flex items-center justify-center" title="Wishlist">
                ...
                <span class="absolute -top-1 -right-1 bg-purple-600 text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold">0</span>
            </a>
            
            <!-- Dynamic Cart Dropdown Menu -->
            <div class="relative" x-data="{ open: false }">
                ...
                <button @click="open = !open" @click.away="open = false" class="relative text-gray-300 hover:text-white p-2.5 rounded-xl bg-[#161626] border border-gray-800 transition flex items-center justify-center">
                    ...
                    <span class="absolute -top-1 -right-1 bg-purple-600 text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold">
                        {{ $headerCartCount }}
                    </span>
                </button>
                ...
                <div x-show="open" 
                    ...
                    <div class="flex items-center justify-between pb-3 border-b border-gray-800">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xs px-2 py-0.5 bg-purple-950 text-purple-300 border border-purple-500/30 rounded-full font-semibold">{{ $headerCartCount }} Items</span>
                        </div>
                        @if($headerCartItems->count() > 0)
                            <form action="{{ route('cart.clear') }}" method="POST">
                                @csrf
                                <button type="submit" class="text-xs text-gray-400 hover:text-red-400 transition">Clear All</button>
                            </form>
                        @endif
                    </div>
                    ...
                    <div class="py-3 max-h-64 overflow-y-auto space-y-3 pr-1 scrollbar-thin scrollbar-thumb-purple-600">
                        @forelse($headerCartItems as $item)
                            <div class="flex items-center justify-between gap-3 bg-[#18182f] p-2.5 rounded-xl border border-gray-800/50">
                                <img src="{{ $item->event_image ?? 'https://images.unsplash.com/photo-1540039155733-5bb30b53aa14?w=600' }}" alt="{{ $item->event_title }}" class="w-14 h-14 object-cover rounded-lg">
                                <div class="flex-1 min-w-0">
                                    <h4 class="text-xs font-bold text-white truncate">{{ $item->event_title }}</h4>
                                    <p class="text-[11px] text-gray-400">Qty: {{ $item->quantity }} x <span class="text-purple-400 font-semibold">৳{{ number_format($item->price) }}</span></p>
                                </div>
                                <form action="{{ route('cart.delete', $item->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="text-gray-400 hover:text-red-400 p-1.5 transition">
                        ...
                        @empty
                            <p class="text-center text-xs text-gray-400 py-6">Your cart is empty.</p>
                        @endforelse
                    </div>
                    ...
                    <div class="pt-3 border-t border-gray-800 space-y-3">
                        <div class="flex justify-between items-center text-sm">
                            ...
                            <span class="font-extrabold text-white text-base">৳{{ number_format($headerCartSubtotal) }}</span>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            ...
                            <button type="button" onclick="alert('Checkout function is currently disabled.')" class="block text-center py-2.5 bg-gray-800/80 text-gray-500 text-xs font-semibold rounded-xl border border-gray-700/50 cursor-not-allowed">
                                Checkout (Off)
                            </button>
                        </div>
                    </div>
            ...
            @auth
                ...
                <div class="relative" x-data="{ profileOpen: false }">
                    ...
                    <div x-show="profileOpen" style="display: none;" class="absolute right-0 mt-3 w-56 bg-[#121222] border border-gray-800/80 rounded-2xl shadow-2xl p-3 z-50 text-white backdrop-blur-xl">
