                ...
                <div class="max-w-4xl bg-[#121222] border border-gray-800 rounded-2xl p-6 shadow-2xl">
                    <form action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="4" placeholder="Write details about the event..." class="w-full bg-[#18182f] text-sm text-gray-200 border @error('description') border-red-500 @else border-gray-800 @enderror rounded-xl px-4 py-3 focus:outline-none focus:border-purple-500 transition">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
